...

{
  resources(
    ...
    local default_api_limits = {
      ...
      // TRC-003/TRC-004: Batch limit restricts number of operations in a single request
      // Helps mitigate duplicate field attacks and alias overloading by limiting batch size
      batch_limit: {
        global: 5,
        per_role: {
          user: 5,
      ...
      rate_limit: {
        global: {
          unique_params: 'IP',
          max_reqs_per_min: 10000,
        },
        per_role: {
          admin: {
            unique_params: 'IP',
            max_reqs_per_min: 20000,
          },
          user: {
            unique_params: ['x-hasura-user-id'],
            max_reqs_per_min: 5000,
          },
          team_member: {
            unique_params: ['x-hasura-user-id'],
            max_reqs_per_min: 5000,
      ...
      [ingress_output_ext]: {
        ...
        metadata: {
          ...
          annotations: {
            ...
            // TRC-003/TRC-004/TRC-005: GraphQL DoS protections via ModSecurity WAF
            'nginx.ingress.kubernetes.io/enable-modsecurity': 'true',
            'nginx.ingress.kubernetes.io/modsecurity-transaction-id': '$request_id',
            // - Limit request body size to 10KB (blocks excessive query length TRC-005)
            // - Block >10 duplicate field names (mitigates TRC-003 duplicate field attacks)
            // - Block >15 field aliases (mitigates TRC-004 alias overloading attacks)
            // - Block queries with >300 GraphQL field selections (addresses deep nesting TRC-002)
            // - Note: Hasura Enterprise API limits provide depth/node/batch/rate limiting
            //   but cannot detect duplicate fields or aliases within a single operation
            'nginx.ingress.kubernetes.io/modsecurity-snippet': |||
              SecRequestBodyLimit 10240
              SecRequestBodyNoFilesLimit 10240
              SecRule REQUEST_BODY "@rx (__typename)(?:[\\s\\n]+__typename){10,}" "id:9999300,phase:2,deny,status:400,log,msg:TRC-003-GraphQL-duplicate-typename-DoS"
              SecRule REQUEST_BODY "@rx alias\\d+:[\\w_]+(?:[\\s\\n]+alias\\d+:[\\w_]+){15,}" "id:9999301,phase:2,deny,status:400,log,msg:TRC-004-GraphQL-alias-overloading-DoS"
              SecRule REQUEST_BODY "@rx (.)\\1{1000,}" "id:9999302,phase:2,deny,status:400,log,msg:TRC-005-GraphQL-character-repetition-DoS"
            |||,
          },
        },
        spec: {
